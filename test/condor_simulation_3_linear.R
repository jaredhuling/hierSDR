...
if ( !(model.num %in% eta.zero.models) )
{
    ...
    semi.hier.phd  <- semi.phd.hier.separate(x.list, drop(y), d = c(1,1,1), h = exp(seq(log(0.5), log(25), length.out = 25)), maxit = 500, maxk = 1200)
    ...
    s.hier.phd.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(s.phd.1$beta, s.phd.3$beta[,1,drop=FALSE])
    s.hier.phd.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(s.phd.2$beta, s.phd.3$beta[,2,drop=FALSE])
    s.hier.phd.betaAB <- cbind(s.hier.phd.betaA[1:nvars,], s.hier.phd.betaB[1:nvars,], s.phd.3$beta[,3,drop=FALSE])

    hier.phd.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(phd.1$beta.hat, phd.3$beta.hat[,1,drop=FALSE])
    hier.phd.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(phd.2$beta.hat, phd.3$beta.hat[,2,drop=FALSE])
    hier.phd.betaAB <- cbind(hier.phd.betaA[1:nvars,], hier.phd.betaB[1:nvars,], phd.3$beta.hat[,3,drop=FALSE])

    hier.sir.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(sir.1$beta.hat, sir.3$beta.hat[,1,drop=FALSE])
    hier.sir.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(sir.2$beta.hat, sir.3$beta.hat[,2,drop=FALSE])
    hier.sir.betaAB <- cbind(hier.sir.betaA[1:nvars,], hier.sir.betaB[1:nvars,], sir.3$beta.hat[,3,drop=FALSE])
...
} else
{
    ...
    semi.hier.phd  <- semi.phd.hier.separate(x.list, drop(y), d = c(1,1,0), h = exp(seq(log(0.5), log(25), length.out = 25)), maxit = 500, maxk = 1200)
    ...
    s.hier.phd.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(s.phd.1$beta, s.phd.3$beta[,1,drop=FALSE])
    s.hier.phd.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(s.phd.2$beta, s.phd.3$beta[,2,drop=FALSE])
    s.hier.phd.betaAB <- cbind(s.hier.phd.betaA[1:nvars,], s.hier.phd.betaB[1:nvars,])

    hier.phd.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(phd.1$beta.hat, phd.3$beta.hat[,1,drop=FALSE])
    hier.phd.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(phd.2$beta.hat, phd.3$beta.hat[,2,drop=FALSE])
    hier.phd.betaAB <- cbind(hier.phd.betaA[1:nvars,], hier.phd.betaB[1:nvars,])

    hier.sir.betaA  <- (diag(nvars * 2) - Pc) %*%  rbind(sir.1$beta.hat, sir.3$beta.hat[,1,drop=FALSE])
    hier.sir.betaB  <- (diag(nvars * 2) - Pc) %*%  rbind(sir.2$beta.hat, sir.3$beta.hat[,2,drop=FALSE])
    hier.sir.betaAB <- cbind(hier.sir.betaA[1:nvars,], hier.sir.betaB[1:nvars,])
