...
result.mat.cor <- result.mat.angle <- data.frame(array(NA, dim = c(1, ncol(grid) + num.models)))

result.mat.cor[1,1:ncol(grid)] <- result.mat.angle[1,1:ncol(grid)] <- grid[sim.idx,]

colnames(result.mat.cor) <-
    colnames(result.mat.angle) <-
    c(colnames(grid), "SIR", "Hier SIR", "PHD", "Hier PHD",
      "Semi PHD", "Semi Hier PHD", "Semi Hier PHD 2")


resul.mat.cor[1,(ncol(grid) + 1):ncol(resul.mat.cor)] <- c(sir.cor, hier.sir.cor,
...
result.mat.angle[1,(ncol(grid) + 1):ncol(result.mat.angle)] <- c(sir.angle, hier.sir.angle,
...
## save results
write.csv(resul.mat.cor,    paste(seed.num, "_results_correlation.csv", sep = ""))
