...
for (n in 1:length(nobs.vec))
{
    ...
    for (s in 1:nsims)
    {
        ...
        sim.subsp.angle.res.list4[[n]][s,1] <- (1/4) * ((angles(Re(hier.sdr$beta.hat[1:nvars,1]), beta.a) + angles(Re(hier.sdr$beta.hat[(nvars+1):(2 * nvars),2]),beta.b) )
                                                        + 2 * angles(Re(hier.sdr$beta.hat[(2*nvars+1):(3 * nvars),]), beta.ab))

        sim.subsp.angle.res.list4[[n]][s,2] <- (1/4) * ((angles(Re(hier.sdr.phd$beta.hat[1:nvars,1]), beta.a) + angles(Re(hier.sdr.phd$beta.hat[(nvars+1):(2 * nvars),2]),beta.b) )
                                                        + 2 * angles(Re(hier.sdr.phd$beta.hat[(2*nvars+1):(3 * nvars),]), beta.ab))

        sim.subsp.angle.res.list4[[n]][s,3] <- (1/4) * ((angles(Re(semi.hier.phd$beta.hat[1:nvars,1]), beta.a) + angles(Re(semi.hier.phd$beta.hat[(nvars+1):(2 * nvars),2]),beta.b) )
                                                        + 2 * angles(Re(semi.hier.phd$beta.hat[(2*nvars+1):(3 * nvars),]), beta.ab))
        ...
        sim.subsp.angle.res.list4[[n]][s,7] <- (1/4) * ((angles(Re(semi.hier.phd2$beta[1:nvars,1]), beta.a) + angles(Re(semi.hier.phd2$beta[(nvars+1):(2 * nvars),2]),beta.b) )
                                                        + 2 * angles(Re(semi.hier.phd2$beta[(2*nvars+1):(3 * nvars),]), beta.ab))

        sim.subsp.angle.res.list4[[n]][s,5] <- (1/4) * ((angles(phd.1$beta.hat[1:nvars,1], beta.a) + angles(phd.2$beta.hat[1:nvars,1],beta.b) )
                                                        + 2 * angles(phd.3$beta.hat, beta.ab))

        sim.subsp.angle.res.list4[[n]][s,4] <- (1/4) * (( angles(sir.1$beta.hat[1:nvars,1], beta.a) + angles(sir.2$beta.hat[1:nvars,1],beta.b) )
                                                        + 2 * angles(sir.3$beta.hat, beta.ab))

        sim.subsp.angle.res.list4[[n]][s,6] <- (1/4) * ((angles(s.phd.1$beta[1:nvars,1], beta.a)+ angles(s.phd.2$beta[1:nvars,1],beta.b) )
                                                        + 2 * angles(s.phd.3$beta, beta.ab))
