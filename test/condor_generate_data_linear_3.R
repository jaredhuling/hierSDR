...
{
    rho <- 0.5
    cov.mat <- rho ^ abs(outer(1:(nvars - 5), 1:(nvars - 5), "-"))


    x.list <- x.list.test <- vector(mode = "list", length = ndata)
    for (g in 1:ndata)
    {
        x.tmp  <- mvrnorm(n = nobs, mu = rep(0, nvars - 5), Sigma = cov.mat)
        ...
        x.list[[g]] <- data.matrix(cbind(x.tmp, x.tmp2))

        x.tmp  <- mvrnorm(n = nobs.test, mu = rep(0, nvars - 5), Sigma = cov.mat)
        x.tmp2 <- apply(x.tmp[,1:5], 2, function(col) rbinom(length(col), 1, 1 / (1 + exp(-col))))
        #x.tmp3 <- apply(x.tmp[,6:10], 2, function(col) 0.1 * col ^ 2 + rnorm(length(col), sd = 1))
        x.list.test[[g]] <- data.matrix(cbind(x.tmp, x.tmp2))
    }
}
...
} else if (model.num == 5)
{
    ...
    y.true.ab <- 5 * apply(sin(x.list[[3]] %*% beta.ab[,2]) , 1, sum) -
        0.5 * ((x.list[[3]] %*% beta.ab[,1]) ^ 2) *  (apply(sin(x.list[[3]] %*% beta.ab[,3]), 1, sum)) ^ 2
    ...
    y.true.ab <- 5 * apply(sin(x.list.test[[3]] %*% beta.ab[,2]) , 1, sum) -
        0.5 * ((x.list.test[[3]] %*% beta.ab[,1]) ^ 2) *  (apply(sin(x.list.test[[3]] %*% beta.ab[,3]), 1, sum)) ^ 2
