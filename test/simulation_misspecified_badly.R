...
for (n in 1:length(nobs.vec))
{
    ...
    for (s in 1:nsims)
    {
        ...
        sdr.sir      <- sir(as.matrix(x), y, d = 1 * 1, h = 30L)
        sdr.phd      <- phd(as.matrix(x), y, d = 1 * 1)

        sir.1 <- sir(x.list[[1]], y[1:nobs], d = 1, h = 30L)
        sir.2 <- sir(x.list[[2]], y[(1 + nobs):(2*nobs)], d = 1, h = 30L)
        sir.3 <- sir(x.list[[3]], y[(1 + 2*nobs):(3*nobs)], d = 1, h = 30L)

        phd.1 <- phd(x.list[[1]], y[1:nobs], d = 1)
        phd.2 <- phd(x.list[[2]], y[(1 + nobs):(2*nobs)], d = 1)
        phd.3 <- phd(x.list[[3]], y[(1 + 2*nobs):(3*nobs)], d = 1)
        ...
        ## beta AB -> eta AB
        hier.sir.cor <- hier.sir.cor + max(sapply(3:3, function(idx) cor.directions(Re(hier.sdr$beta.hat[(2*nvars+1):(3 * nvars),3]),     beta.ab[,idx], x.list.test[[3]])))
        hier.phd.cor <- hier.phd.cor + max(sapply(3:3, function(idx) cor.directions(Re(hier.sdr.phd$beta.hat[(2*nvars+1):(3 * nvars),3]), beta.ab[,idx], x.list.test[[3]])))
        phd.cor      <- phd.cor      + max(sapply(3:3, function(idx) cor.directions(phd.3$beta.hat[1:nvars,1], beta.ab[,idx], x.list.test[[3]])))
        sir.cor      <- sir.cor      + max(sapply(3:3, function(idx) cor.directions(sir.3$beta.hat[1:nvars,1], beta.ab[,idx], x.list.test[[3]])))

        hier.sir.cor <- hier.sir.cor / 3
        hier.phd.cor <- hier.phd.cor / 3
        phd.cor      <- phd.cor / 3
        sir.cor      <- sir.cor / 3
...
pdf(paste0(fig.path, "sim_2a_nvars100_badly_misspecified_boxplots.pdf"), height = 8, width = 10)
...
pdf(paste0(fig.path, "sim_2a_directions_nvars100_badly_misspecified_boxplots.pdf"), height = 8, width = 10)
...
pdf(paste0(fig.path, "sim_2b_nvars50_badly_misspecified_boxplots.pdf"), height = 8, width = 10)
...
pdf(paste0(fig.path, "sim_2b_directions_nvars50_badly_misspecified_boxplots.pdf"), height = 8, width = 10)
